 ...
 * sibling to a `page.tsx` becomes that route's Suspense boundary) -- no
 * manual <Suspense> needed here.
 */
    ...
    <PageShell>
      <div className="mx-auto w-full max-w-5xl" aria-busy="true" aria-label="Loading markets">
        ...
        <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="h-11 w-full animate-pulse rounded-xl bg-surface sm:max-w-xs" />
          <div className="flex gap-2">
            {Array.from({ length: 3 }).map((_, index) => (
              <div key={index} className="h-9 w-24 animate-pulse rounded-full bg-surface" />
            ))}
          </div>
        </div>
          ...
          {Array.from({ length: SKELETON_ROW_COUNT }).map((_, index) => (
            <div key={index} className="flex items-center gap-4 px-4 py-3 sm:px-6">
              ...
              <div className="min-w-0 flex-1 space-y-2">
                ...
                <div className="h-3 w-16 animate-pulse rounded-full bg-surface-hover" />
              </div>
              <div className="hidden h-4 w-16 animate-pulse rounded-full bg-surface-hover sm:block" />
